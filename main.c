...
struct Clientes{//cria a struct principal
    char nome[50],telefone[18],rg[13];
    int flag;
}clientes[100];

int main() {
    struct Clientes puxa;
    int numClientes=0, i, escolha,num,opcao;//declara as variaveis necessarias
    do{
    printf(" -- MANTER CLIENTE -- AEROPORTO DE TOLEDO --\n");
    arquivo = fopen("clientes.bin", "rb");//abre o arquivo para que leia os clientes no bin
    if (arquivo == NULL) {//verifica se o bin existe
        printf("Erro ao abrir o arquivo.\n");
    }
    fread(&num,sizeof(numClientes),1,arquivo);//le o numero de clientes no bin
    printf("\nLista de clientes:\n");
    for (i = 1; i < numClientes+1; i++) {//printa todos os clientes
        fread(&puxa,sizeof(struct Clientes[i]),1,arquivo);//le cada um dos clientes
        if(clientes[i].flag!=1){//apenas printa o cliente se ele nao esta oculto
        printf("%d- %s\n", i , clientes[i].nome);
        }
    }
    ...
    case 1://cadastra um novo cliente
        ...
        fflush(stdin);
        gets(clientes[numClientes].nome);//recebe um nome para o novo cliente
        printf("Digite o RG: ");
        fflush(stdin);
        gets(clientes[numClientes].rg);//recebe um rg para o novo cliente
        printf("Digite o telefone: ");
        fflush(stdin);
        gets(clientes[numClientes].telefone);//recebe um telefone para o novo cliente
    ...
    case 2://incia a compra de uma passagem
        ...
        fflush(stdin);
        printf("\nCliente escolhido:\n Nome: %s\n Telefone: %s\n RG: %s\n\n", clientes[escolha].nome, clientes[escolha].telefone, clientes[escolha].rg);
        ...
        fprintf(pont_arq,"---VIA DO CLIENTE---\n Cliente: %s \n Regristro geral: %s \n",clientes[escolha].nome,clientes[escolha].rg);//escreve no txt
        ...
        arquivo = fopen("clientes.bin", "rb");//abre o bin
        fprintf(pont_arq,"\n8<\n------------------------------------\n\n---COLE NA BAGAGEM---\n");
        fprintf(pont_arq," Nome do passageiro: %s \n Telefone: %s \n RG: %s",clientes[escolha].nome, clientes[escolha].telefone, clientes[escolha].rg);
        fclose(arquivo);//fecha o bin
        fclose(pont_arq);//fecha o txt
    break;
    case 3://edita informacoes do cliente
        ...
        switch(opcao){
            case 1://escolhe o nome
                ...
                fflush(stdin);
                gets(clientes[escolha].nome);//pega a nova informacao
                fwrite(&puxa, sizeof(clientes[escolha].nome), 1, arquivo);//salva a nova informacao
            ...
            case 2://escolhe telefone
                ...
                fflush(stdin);
                gets(clientes[escolha].telefone);//pega a nova informacao
                fwrite(&puxa, sizeof(clientes[escolha].telefone), 1, arquivo);//salva a nova informacao
            ...
            case 3://escolhe rg
                ...
                fflush(stdin);
                gets(clientes[escolha].rg);//pega a nova informacao
                fwrite(&puxa, sizeof(clientes[escolha].rg), 1, arquivo);//salva a nova informacao
    ...
    case 4://exclui um cliente
        ...
        clientes[escolha].flag=1;//muda a flag do cliente para que ele nao seja exibido na lista de clientes
        fwrite(&puxa, sizeof(clientes[escolha].flag), 1, arquivo);//atribue o novo valor de flag no bin
